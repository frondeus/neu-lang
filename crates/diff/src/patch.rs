...
impl<'a> Comparison<'a> {
    pub fn patch(&self, left_name: Cow<'a, str>, left_dt: &'a DelayedFormat<StrftimeItems<'a>>,
                 right_name: Cow<'a, str>, right_dt: &'a DelayedFormat<StrftimeItems<'a>>, options: PatchOptions) -> ComparisonPatch {
        ComparisonPatch { comparison: self, left_name, left_dt, right_name, right_dt, options }
    }
}
...
impl<'a> fmt::Display for ComparisonPatch<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for hunk in self.comparison.hunks().unwrap() {
