...
*/
use colored::*;
use crate::{Line, LineKind, Hunk, Comparison};
use std::fmt;
...
impl<'a> Line<'a> {
    pub fn display(&self, options: DisplayOptions) -> LineDisplay {
        LineDisplay { line: self, options }
    }
}
...
impl<'a> fmt::Display for LineDiff<'a> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let diff = crate::Comparison { left: &l, right: &r, context_radius: len }.hunks().unwrap();
        if diff.is_empty() {
...
impl<'a> Hunk<'a> {
    pub fn display(&self, options: DisplayOptions) -> HunkDisplay {
        HunkDisplay { hunk : self, options }
    }
}
...
impl<'a> Comparison<'a> {
    pub fn display(&self, options: DisplayOptions) -> ComparisonDisplay {
        ComparisonDisplay { comparison : self, options }
    }
}
...
impl<'a> fmt::Display for ComparisonDisplay<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let result = self.comparison.hunks().unwrap();
        if !result.is_empty() {
            let mut msg = String::from("\n");

            msg += &result.into_iter().map(|s| s.display(self.options).to_string()).join("\n");
